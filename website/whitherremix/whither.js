...
( function () {
	...
	var finalcompletions = 0; // How many times has level 20 been finished?
	var finaldelta = 0;
	var change = 5;
	var real_complete = false;
	...
	var foot_5 = "foot_5";
	var ending = "ending";
	var path = "http://users.wpi.edu/~hjwheelermackta/Frauds/whitherremix/"
	...
	function init_true_goal(){
		...
		PS.color( gx2, gy2, COLOR_GOAL );
		PS.border( gx2, gy2, WIDTH_GOAL_BORDER );
		PS.borderColor( gx2, gy2, COLOR_GOAL_BORDER );
		PS.borderAlpha( gx2, gy2, finaldelta);
		if(finaldelta == 255){
			PS.data( gx2, gy2, IS_GOAL ); // set id
			play = false;
			grow_food();
		}
	...
	function audio_init(){
		PS.audioLoad(foot_1, {lock: true, path: path});
		PS.audioLoad(foot_2, {lock: true, path: path});
		PS.audioLoad(foot_3, {lock: true, path: path});
		PS.audioLoad(foot_4, {lock: true, path: path});
		PS.audioLoad(foot_5, {lock: true, path: path});
		PS.audioLoad(ending, {lock: true, path: path});
	}
	function grow_food(){
		var foods = [[1, 7], [2, 7], [3, 7], [4, 7], [4, 6], [4, 5], [4, 4], [4, 3], [5, 3], [6, 3], [7, 3], [8,3]];
		var timerF;
		var i = 0;
		real_complete = true;
		PS.audioStop(whither);
		timerF = PS.timerStart(15, function(){
			makeFood(foods[i][0], foods[i][1]);
			var foot = PS.random(5);
			PS.audioPlay("foot_" + foot, {path: path});
			i++;
			if(i == 12){
				PS.timerStop(timerF);
				play = true;
	...
	function play_ending() {
		...
		var path = "http://users.wpi.edu/~hjwheelermackta/Frauds/whitherremix/"
		PS.audioPlay("ending", {path: path});
		PS.fade(9, 3, 200);
		PS.color(9, 3, PS.COLOR_WHITE);

		var timer3 = PS.timerStart(200, function(){
			PS.timerStop(timer3);
		...
		var timer4 = PS.timerStart(400, function(){
			PS.timerStop(timer4);
			PS.statusFade(150);
	...
	function blackout () {
		...
		if(level == 20){
			finalcompletions++;
			var timer2 = PS.timerStart(61, function(){
				PS.timerStop(timer2);
				init_true_goal();
				finaldelta = finaldelta + change;
				change += 10;
				if(change == 75){
					finaldelta = 255;
				}
